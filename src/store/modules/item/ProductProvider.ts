...
export class ProductProvider extends PsProvider {
    ...
    async submitItemEntry(holder: ItemEntryParameterHolder): Promise<PsResource<Product>> {
        this.loading.value = true;
        this.product = await PsApiService.postItemEntry<Product>(new Product(), holder.toMap());
        this.loading.value = false;
    ...
    async postItemImageUpload(imgParentId : string, imgType : string, ordering : Number, imgDesc : string, imageFile: File, imgId : string) : Promise<PsResource<DefaultPhoto>> {
        this.loading.value = true;
        
        const returnData = await PsApiService.postItemImageUpload<DefaultPhoto>(new DefaultPhoto(), imgParentId, imgType, ordering, imgDesc, imageFile, imgId);
    ...
    async postItemVideoUpload(imgParentId : string, videoFile: File) : Promise<PsResource<DefaultPhoto>> {
        this.loading.value = true;
        
        const returnData = await PsApiService.postItemVideoUpload<DefaultPhoto>(new DefaultPhoto(), imgParentId, videoFile);
    ...
    async postItemVideoThumbUpload(imgParentId : string, videoThumbFile: File) : Promise<PsResource<DefaultPhoto>>  {
        this.loading.value = true;
        
        const returnData = await PsApiService.postItemVideoThumbUpload<DefaultPhoto>(new DefaultPhoto(), imgParentId, videoThumbFile);
        this.loading.value = false;
