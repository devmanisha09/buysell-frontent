            ...
            <div class="relative flex flex-col " style="height:90vh" > 
                ...
                <div class='lg:mt-32 mt-28 h-40 flex flex-col ' >
                    ...
                    <div v-if="dataReady" class='flex flex-row justify-between'>
                        <ps-route-link :to="{ name: 'item', params: { itemName: itemName.split(' ').join('-').toLowerCase() }, query: { item_id: itemId, item_name: itemName.split(' ').join('-').toLowerCase()   }}">
                            <div class='flex flex-row cursor-pointer'>
                                ...
                                <div class='flex flex-col ms-4 mt-2'>
                                    ...
                                    <ps-label class='mt-1'> {{ currency }} {{ formatPrice(itemPrice) }} ( {{condition}} )  </ps-label>
                        ...
                        <div v-if="chatHistoryProvider.chatHistory.data?.item.isSoldOut == '1'" class="flex flex-col content-end items-center">
                            <div >
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_BOUGHT">
                                ...
                                <div  class="w-full flex flex-col justify-center items-center">
                                    ...
                                    <div v-if='chat.sendByUserId == loginUserId && chat.offerStatus == PsConst.CHAT_STATUS_IS_USER_BOUGHT'>
                                        <ps-label @click="markAsSold(chat)" class="p-2 flex flex-shrink-0 bg-primary-500 dark:bg-primaryDark-accent cursor-pointer rounded-lg" textColor="text-textLight dark:text-textDark"> {{ $t("chat__mark_as_sold") }} </ps-label>
                                    </div>
                                </div>
...
export default {
    ...
    setup() { 
        ...
        itemPrice = format(PsConfig.priceFormat, parseFloat(itemPrice)).toString();
        ...
        function formatPrice(value) {
            if(value.toString() == '0') {
                ...
                return format(PsConfig.priceFormat, parseFloat(value))
            }
        }
        ...
        const userRef = firebaseApp.database().ref('Current_Chat_With');
        let otherUserId;
        ...
        onMounted( async () => {
            ...
            userRef.child(otherUserId).on('value', (snapshot) => {
                ...
                if(snapshot.val() != null){
                    ...
                    const tmpItemId = snapshot.val().itemId;
                    if(tmpItemId == itemId) {
