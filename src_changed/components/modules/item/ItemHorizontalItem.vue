        ...
        <ps-card class="flex w-full flex-col " :enabledHover="true">
            ...
            <div class="lg:px-2 lg:py-2 px-1 py-1 bg-primary-000 dark:bg-primaryDark-black" :class="psValueHolder.showProfile == 'show' ? 'rounded-b-xl lg:rounded-b-2xl pt-0' : 'rounded-xl lg:rounded-2xl'">
                <div class="relative lg:h-56 h-36 ">
                    <div class="absolute ">
                        <img alt="Placeholder" class="rounded-lg block object-cover w-screen lg:h-56 h-36 " width="640px" height="360px" v-lazy="productProvider.imageUrl(item ? item.defaultPhoto.imgPath : '') " @error="productProvider.defaultCarImage"/>
                    </div>
                ...
                <div class="flex flex-row pt-2 ps-1 inline-flex">
                    <ps-label class=" truncate font-medium text-sm lg:text-base" > {{item ? item.title : ''}} </ps-label>
                    <ps-label class="ms-2 flex-grow text-xxs lg:text-xs flex font-semibold" textColor="text-primary-500 dark:text-primaryDark-accent font-semibold"> {{item ? item.conditionOfItem.name : ''}} </ps-label>
                </div>
                <ps-label class="pt-1 ps-1 font-medium text-xs lg:text-base bg-pri" textColor=" text-primary-500 dark:text-primaryDark-accent"> {{ item ? item.itemCurrency.currencySymbol : '' }} {{ formatPrice(item ? item.price : '') }} </ps-label>
